/*
 * test_envol.c — les règles du jeu, sans écran.
 *
...
static bool finite_game(const envol *g)
{
    if (!isfinite(g->bird_y) || !isfinite(g->bird_vy) || !isfinite(g->bird_angle)) return false;
    for (int i = 0; i < ENV_PIPES; ++i) if (!isfinite(g->pipes[i].position)) return false;
    return true;
}
...
{
    envol g;
    envol_reset(&g, 1234, false);
    CHECK(g.phase == ENV_READY, "on démarre en attente");
    ...
    for (int i = 0; i < 1200; ++i) envol_tick(&g, DT);
    CHECK(g.phase == ENV_READY, "sans battement, la partie ne commence pas");
...
{
    envol g;
    envol_reset(&g, 7, false);
    envol_flap(&g);
    CHECK(g.phase == ENV_PLAYING, "un battement lance la partie");
...
static void test_gap_is_passable(void)
{
    ...
    for (int hard = 0; hard < 2; ++hard) {
        envol g;
        envol_reset(&g, 99 + (uint64_t)hard, hard != 0);

        int steps = 0;
        while (g.phase != ENV_DEAD && steps < 120 * 60) {   /* une minute */
            envol_autopilot(&g);
            envol_tick(&g, DT);
            steps++;
        }
...
static void test_pipes_stay_spaced(void)
{
     ...
     */
    envol g;
    envol_reset(&g, 4242, false);
    for (int i = 0; i < 120 * 45; ++i) { envol_autopilot(&g); envol_tick(&g, DT); }

    float xs[ENV_PIPES];
    for (int i = 0; i < ENV_PIPES; ++i) xs[i] = g.pipes[i].position;
    for (int i = 0; i < ENV_PIPES; ++i) {
        for (int j = i + 1; j < ENV_PIPES; ++j) {
 ...
 * Envol était une DROITE. Pilote automatique, cinq graines, six durées — 1, 4,
...
static void test_la_difficulte_monte(void)
{
    ...
    g.phase = ENV_PLAYING;     /* le décor ne défile qu'en jeu */
    g.score = 100;
    for (int i = 0; i < ENV_PIPES; ++i) g.pipes[i].position = -1e4f;
    envol_tick(&g, DT);          /* le recyclage replace les huit tuyaux */
...
{
    envol g;
    envol_reset(&g, 8, false);
    uint32_t events = 0;
    for (int i = 0; i < 120 * 30; ++i) {
        envol_autopilot(&g);
        envol_tick(&g, DT);
        if (g.scored_now) events++;
        if (g.phase == ENV_DEAD) break;
    }
...
{
    envol g;
    envol_reset(&g, 3, false);
    envol_flap(&g);

    /* On ne fait plus rien : l'oiseau doit toucher le sol et mourir. */
    int steps = 0;
    while (g.phase != ENV_DEAD && steps < 120 * 20) { envol_tick(&g, DT); steps++; }
    CHECK(g.phase == ENV_DEAD, "sans battement, on finit par mourir");
    CHECK(g.died_now || steps > 0, "la mort est signalée");

    /* Puis la chute finale s'arrête AU sol, elle ne le traverse pas. */
    for (int i = 0; i < 600; ++i) envol_tick(&g, DT);
    CHECK(finite_game(&g), "aucun NaN après la mort");
    CHECK(g.bird_y < ENV_H, "l'oiseau ne traverse pas le sol (y = %.1f)", (double)g.bird_y);
    ...
    const envol_phase before = g.phase;
    envol_flap(&g);
...
static void test_deterministic(void)
{
     ...
     */
    envol a, b;
    envol_reset(&a, 20240418, false);
    envol_reset(&b, 20240418, false);
    for (int i = 0; i < 120 * 20; ++i) {
        envol_autopilot(&a); envol_tick(&a, DT);
        envol_autopilot(&b); envol_tick(&b, DT);
    }
    ...
    envol c;
    envol_reset(&c, 777, false);
    for (int i = 0; i < 120 * 20; ++i) { envol_autopilot(&c); envol_tick(&c, DT); }
